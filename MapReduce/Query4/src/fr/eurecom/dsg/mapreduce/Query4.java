...
class CompositeKey implements WritableComparable {
	...
	public String toString() {
		return (new StringBuilder()).append(carrier).toString();
	}
...
IntWritable> { //output value type
	...
	protected void map(LongWritable key, //input key type
		...
		if(words.length > 8){
			...
			if(isNotEmpty(words[1]) && isNotEmpty(words[2]) && isNotEmpty(words[6]) && isNotEmpty(words[7]) && isNotEmpty(words[8]) 
					&& isNumber(words[1]) && isNumber(words[2]) && isNumber(words[6]) && isNumber(words[7])){
